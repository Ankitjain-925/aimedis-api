        ...
        User.findOne({ _id: legit.id }, function (err, userdata) {
            if (err) {
                res.json({ status: 200, hassuccessed: false, message: "user not found", error: err })
                    ...
                    console.log('I am here2', userdata)
                    User.find({ profile_id: req.params.id, fav_doctor: { $elemMatch: { doctor: userdata.profile_id } } }, function (err2, userdata2) {
                        if (err2) {
                            ...
                            if (userdata2.length != 0) {
                                ...
                                console.log('i AM HERE', req.params.id, userdata.profile_id)
                                var data = {doctor : userdata.profile_id, profile_id : userdata.profile_id };
                                User.findOneAndUpdate({ profile_id: req.params.id }, { parent_id:  legit.id, $push: { fav_doctor: data} },{upsert:true}, function (err2, updatedata) {
                                    if (err2 && !updatedata) {
                                        res.json({ status: 200, hassuccessed: false, message: "something went wrong", error: err })
                                    } else {
                                        res.json({ status: 200, hassuccessed: true, message: "user updated", data: userdata })
            ...
            User.findOne({$or: [{ email: req.params.id },{
                ...
                .then((user_data1) => {
                    if (user_data1) {
                        ...
                        console.log('sdfs', user_data1.email)
                        var mailOptions = {
                            from: "contact@aimedis.com",
                            to: user_data1.email,
