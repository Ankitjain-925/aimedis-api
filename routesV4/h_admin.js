...
router.delete('/AddGroup/:institute_id/:group_id', function (req, res, next){
    ...
    let legit = jwtconfig.verify(token)
    if (legit) {
        Institute.updateOne({ _id: req.params.institute_id }, { $pull: { institute_groups: { _id: req.params.group_id  } } },
        { multi: true },
...
router.put('/associatedTo/:userid', function (req, res, next) {
    ...
    let legit = jwtconfig.verify(token)
    if (legit) {
        User.find({ _id: req.params.userid, assosiated_by: { $elemMatch: { institute_id: req.body.institute_id } } }, function (err2, userdata2) {
            if (err2) {
                res.json({ status: 200, hassuccessed: false, message: "something went wrong", error: err2 })
            } else {
                if (userdata2.length != 0) {
                    res.json({ status: 200, hassuccessed: false, message: "Already associated with the institute", error: err2 })
                } else {
                    User.findOneAndUpdate({_id: req.params.userid }, { $push: { assosiated_by: req.body } }, { upsert: true }, function (err2, updatedata) {
                        if (err2 && !updatedata) {
                            res.json({ status: 200, hassuccessed: false, message: "something went wrong", error: err2 })
                        } else {
                            res.json({ status: 200, hassuccessed: true, message: "user associated with institute successfully" })
                        }
                    })
                }
            }
...
router.delete('/associatedTo/:user_id/:institute_id', function (req, res, next) {
    const token = (req.headers.token)
    let legit = jwtconfig.verify(token)
    if (legit) {
...
router.put('/assignedHouse/:userid', function (req, res, next){
    ...
    let legit = jwtconfig.verify(token)
    if (legit) {
        User.find({ _id: req.params.userid, houses: { $elemMatch: { value: req.body.value } } }, function (err2, userdata2) {
            if (err2) {
                res.json({ status: 200, hassuccessed: false, message: "something went wrong", error: err2 })
            } else {
                if (userdata2.length != 0) {
                    res.json({ status: 200, hassuccessed: false, message: "House already exist", error: err2 })
                } else {
                    User.findOneAndUpdate({_id: req.params.userid }, { $push: { houses: req.body } }, { upsert: true }, function (err2, updatedata) {
                        if (err2 && !updatedata) {
                            res.json({ status: 200, hassuccessed: false, message: "something went wrong", error: err2 })
                        } else {
                            res.json({ status: 200, hassuccessed: true, message: "House is assigned" })
                        }
                    })
                }
            }
...
router.delete('/assignedHouse/:userid/:house_id', function (req, res, next){
    ...
    let legit = jwtconfig.verify(token)
    if (legit) {
        User.updateOne({ _id: req.params.userid }, { $pull: { houses: { value: req.params.house_id  } } },
        { multi: true },
        function (err, userdata) {
            if (err) {
                res.json({ status: 200, hassuccessed: false, message: 'Something went wrong.', error: err });
