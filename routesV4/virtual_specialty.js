  ...
  if (legit) {
    ...
    virtual_Case.find({ patient_id: req.params.patient_id }, function (err, data) {
      if (err & !data) {
        ...
        if (data && data.length > 0) {
          taskfromhouseid(data).then((data1) => {

            data.map((item) => {
              if (item.inhospital == false) {
                console.log("1")
                Promise.all([ansfromhouseid(data), invoicefromhouseid(data)]).then((final_data) => {
                  console.log("2")
                  var combine_data = { ...data1, ...final_data }
                  var flatArray = Array.prototype.concat.apply([], combine_data);
                  Array_flat.push(flatArray)

                  console.log("flatArray", flatArray)
                  // flatArray.sort(mySorter);
                })
                res.json({ status: 200, hassuccessed: true, message: 'succefully find', data: Array_flat })

              } else {

                res.json({ status: 200, hassuccessed: true, message: 'succefully find', data: data1 })
              }
            })
          })
        }
    ...
    data.map((item) => {
      ...
      virtual_Task.find({ $or: [{ house_id: item.house_id }, { house_id: VirtualtToSearchWith.house_id }] }).exec(function (err, task) {
        if (err) {
          ...
          Institute.find({ "institute_groups.houses.house_id": item.house_id }).exec(function (err, task2) {
            if (err) {
              reject(err)
            } else {
              console.log("task2", task2)
