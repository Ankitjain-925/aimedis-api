          ...
          if (doc && doc.length > 0) {
            ...
            promise.then((upcoming_appointment) => {
              ...
              doc[0].track_record.forEach(function (part, index) {
                part["created_by"] =
                  part._enc_created_by === true
                    ? decrypt(part.created_by)
                    : part.created_by;
              });
              if (doc[0].track_record.length > 0) {
                var myFilterData1 = doc[0].track_record.filter(
                  (value, key) => value.type === "diagnosis"
                );
                last_dia = myFilterData1[0];

                var myFilterData2 = doc[0].track_record.filter(
                  (value, key) => value.type === "doctor_visit"
                );
                last_dv = myFilterData2;
                if (myFilterData2.length > 2) {
                  last_dv = myFilterData2.filter((value, key) => key < 2);
                }

                var myFilterData3 = doc[0].track_record.filter(
                  (value, key) => value.type === "condition_pain"
                );
                last_con = myFilterData3[0];

                var myFilterData4 = doc[0].track_record.filter(
                  (value, key) => value.type === "weight_bmi"
                );
                weight_bmi = myFilterData4;

                var myFilterData5 = doc[0].track_record.filter(
                  (value, key) => value.type === "prescription"
                );
                prescriptions = myFilterData5;
                if (myFilterData5.length > 2) {
                  prescriptions = myFilterData5.filter((value, key) => key < 2);
                }

                var myFilterData6 = doc[0].track_record.filter(
                  (value, key) => value.type === "sick_certificate"
                );
                sick_certificates = myFilterData6;
                if (myFilterData6.length > 2) {
                  sick_certificates = myFilterData6.filter(
                    (value, key) => key < 2
                  );
                }
                var myFilterData7 = doc[0].track_record.filter(
                  (value, key) => value.type === "blood_pressure"
                );
                blood_pressure = myFilterData7;

                var myFilterData9 = doc[0].track_record.filter(
                  (value, key) => value.type === "blood_sugar"
                );
                blood_sugar = myFilterData9;

                var myFilterData8 = doc[0].track_record.filter(
                  (value, key) => value.type === "laboratory_result"
                );
                laboratory_result = myFilterData8;

                var myFilterData23 = doc[0].track_record.filter(
                  (value, key) => value.type === "respiration"
                );
                respiration = myFilterData23;
              }
              if (last_dv && last_dv.length > 0) {
                forEachPromise(last_dv, getAlltrack).then((result) => {
                  info = {
                    birthday: doc[0].birthday,
