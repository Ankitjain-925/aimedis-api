...
router.get('/UpcomingAppintmentDoc', function (req, res, next) {
    ...
    if (legit) {
        ...
        date = new Date(date);
        Appointment.find({
            $and: [{ $or: [{ doctor_id: messageToSearchWith.doctor_id }, { doctor_id: legit.id }] },
            {
                $or: [
                    { date: { $gte: date, } },
                    { date: { $eq: date, } }
...
router.get('/UpcomingAppintmentPat', function (req, res, next) {
    ...
    if (legit) {
        ...
        date = new Date(date);
        Appointment.find({
            $and: [{ $or: [{ patient: messageToSearchWith.patient }, { patient: legit.id }] },
            {
                $or: [
                    { date: { $gte: date, } },
                    { date: { $eq: date, } }
...
router.get('/UpcomingAppintmentPat/:Userid', function (req, res, next) {
    ...
    if (legit) {
        ...
        date = new Date(date);
        Appointment.find({
            $and: [{ $or: [{ patient: messageToSearchWith.patient }, { patient: req.params.Userid }] },
            {
                $or: [
                    { date: { $gte: date, } },
                    { date: { $eq: date, } }
            ...
            .exec(function (err, doc) {
                if (err && !dofc) {
                    ...
                    if (doc == null || doc == 'undefined') {
                        ...
                        lan1.then((result) => {
                            result = result === 'ch' ? 'zh' : result === 'sp' ? 'es' : result === 'rs' ? 'ru' : result;
                            var sms1 = 'There was an emergency an access to the data in your Aimedis profile ( ' + doc.profile_id + ' ) by Doctor - ' + req.body.current_info.profile_id + ' - ' + req.body.current_info.first_name + ' ' + req.body.current_info.last_name + ' on ' + dateString;
                            trans(sms1, { source: "en", target: result }).then((res1) => {
                                sendSms(doc.mobile, res1).then(result => {
                            ...
                            if (doc.emergency_number && doc.emergency_number !== '') {
                                var sms2 = 'There was an emergency access to the data in' + doc.first_name + ' ' + doc.last_name + ' Aimedis profile ( ' + doc.profile_id + ' ) by Doctor - ' + req.body.current_info.profile_id + ' - ' + req.body.current_info.first_name + ' ' + req.body.current_info.last_name + ' on ' + dateString;
                                trans(sms2, { source: "en", target: result }).then((res1) => {
                                    sendSms(doc.emergency_number, res1).then(result => {
...
router.post('/AskPatient/:id', function (req, res, next) {
    ...
    if (legit) {
        ...
        User.findOne({
            type: 'patient', $or: [
                { alies_id: messageToSearchWith1.alies_id }, { profile_id: messageToSearchWith.profile_id }, { profile_id: req.params.id }, { alies_id: req.params.id }]
        }).exec()
            .then((user_data1) => {
                if (user_data1) {
