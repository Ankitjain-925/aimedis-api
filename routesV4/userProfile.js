...
router.post("/MarketingSub",function(req,res){
  ...
  marketing_user.findOne({$or: [
    ...
    if(err){
      ...
      data.first_name = base64.encode( data.first_name );
      data.last_name= base64.encode(data.last_name);

      sendData = `Dear User,<br/>
      You registered as a newsletter subscription, So for getting updates related to us. Please go to the LINK https://aimedis.io/newsletter-approval?fisrt_name=${ data.first_name}&last_name=${ data.last_name}&email=${email},  and do final step for the substcription.`;
      generateTemplate(
        EMAIL.generalEmail.createTemplate("en", {
          title: "",
          content: sendData,
