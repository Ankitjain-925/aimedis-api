...
function sendSms(to = "", message = "", countrycode = "") {
  let reciver = to.split("-");
  let mobile;
  if(to.includes("+")){
    mobile=to;
  }
...
if (reciver.length == 2) {
  let code = countries.filter((obj) => {
    return obj.code.toLowerCase() == reciver[0].toLowerCase();
  })[0].dial_code;

  mobile = `${code}${reciver[1]}`;
} else {
  let code = countries.filter((obj) => {
    return (
      obj.code.toLowerCase() == countrycode.toLowerCase() ||
      obj.dial_code == countrycode
    );
  })[0].dial_code;
  mobile = `${code}${to}`;
}
